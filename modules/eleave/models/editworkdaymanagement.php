<?php
/**
 * @filesource modules/eleave/models/editworkdaymanagement.php
 *
        ...
        if ($request->initSession() && $request->isSafe() && $login = Login::isMember()) {
            if (Login::notDemoMode($login) && Login::checkPermission($login, 'can_manage_eleave')) {
                try {
                    // ค่าที่ส่งมา
                    $save = array(
                        'member_id' => $request->post('member_id')->topic(),
                        'year' => $request->post('year')->date(), // แก้ไขเป็น text แทน textarea
                    ...
                    if (!$index) {
                        ...
                        if (empty($ret)) {
                            if ($index->id == 0) {
                                // ใหม่
                                $index->id = $this->db()->insert($this->getTableName('shift_workdays'), $save);
                            } else {
                                // แก้ไข
                                $this->db()->update($this->getTableName('shift_workdays'), $index->id, $save);
                            }
                            // log
                            \Index\Log\Model::add($index->id, 'eleave', 'Save', '{LNG_Leave type} ID : '.$index->id, $login['id']);
                            // คืนค่า
                            $ret['alert'] = Language::get('Saved successfully');
                            $ret['location'] = $request->getUri()->postBack('index.php', array('module' => 'eleave-workdaymanagement'));
                            // เคลียร์
                            $request->removeToken();
                        }
                    }
